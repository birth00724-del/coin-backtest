...
import pandas as pd
import numpy as np
from scipy.optimize import minimize  # (현재 예제에선 사용 X, 확장 대비)
import plotly.graph_objects as go

st.set_page_config(page_title="업비트 백테스터", layout="wide")
st.title("📈 업비트 전략 백테스터")
st.caption("CSV 업로드 후 전략/슬리피지/기간을 선택하세요. (필수 열: Date, Close, Volume)")
...
def metrics_from_curve(curve: pd.Series, rf_daily: float = 0.0001):
    ...
    cagr = (final_cap / curve.iloc[0]) ** (1 / n_years) - 1 if n_years > 0 else np.nan
...
fig.update_layout(
    title=f"누적 자산곡선 (초기자금 {initial_capital:,.0f}, 슬리피지 {slippage_pct}%, 최근 {years}년)",
    xaxis_title="날짜",
    yaxis_title="자산가치",
...
st.success("완료! 전략/슬리피지/기간을 바꿔가며 결과를 비교해보세요.")
